...
exports.createTodoList = async (req, res) => {
    try {
        ...
        await TodoListModel.createTodoList(email, todoData);
        res.json({ status: "success", message: "To-Do List item created successfully" });
    } catch (err) {
        res.json({ status: "fail", message: err });
    }
}
...
exports.updateTodoList = async (req, res) => {
    try {
        ...
        await TodoListModel.updateTodoList(email, todoId, updatedTodoData);
        res.json({ status: "success", message: "To-Do List item updated successfully" });
    } catch (err) {
        res.json({ status: "fail", message: err });
    }
}
...
exports.deleteTodoList = async (req, res) => {
    try {
        let email = req.headers['email'];
        let todoId = req.params.todoId;
        await TodoListModel.deleteTodoList(email, todoId);
        res.json({ status: "success", message: "To-Do List item deleted successfully" });
    } catch (err) {
        res.json({ status: "fail", message: err });
    }
}
...
exports.markTodoList = async (req, res) => {
    try {
        let email = req.headers['email'];
        let todoId = req.params.todoId;
        let status = req.body.status; 
        await TodoListModel.markTodoList(email, todoId, status);
        res.json({ status: "success", message: "To-Do List item marked successfully" });
    } catch (err) {
        res.json({ status: "fail", message: err });
    }
}
